...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/25 23:28:51 by kfumiya           #+#    #+#             */
/*   Updated: 2021/02/01 07:00:14 by kfumiya          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static int	format_value(char **str, char format, va_list *args, t_flags *flags)
{
	if (format == 'd' || format == 'i')
		...
		return (malloc_hex_lx(str, format, va_arg(*args, unsigned int), flags));
	else if (format == 'x')
		return (malloc_hex_sx(str, format, va_arg(*args, unsigned int), flags));
	else if (format == 'c')
...
static char	*process_format(char *str, char f, int *length, va_list *args)
{
	t_flags	*flags;
	char	*ret;
...
static char	*add_format_string(t_str **result, char const *str,
...
static int	parse_str(char const *str, int i, t_str **result, va_list *args)
{
	int	start;
